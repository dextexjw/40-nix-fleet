...
printf 'Checking migrated testbed app units are absent from productivity-vm...\n'
for unit in redis-affine.service podman-affine.service affine-postgresql-extensions.service affine-postgresql-password.service gitea.service gitea-oidc-config.service phpfpm-firefly-iii.service firefly-iii-cron.timer stirling-pdf.service on-demand-apps-dashboard.service; do
  colmena exec --on "$HOST" -- "test \"\$(systemctl show -P LoadState '$unit' 2>/dev/null || true)\" = not-found"
done
...
if allow_missing_unit garage-plane-bucket.service; then
  printf 'Skipping Plane Garage bucket checks because garage-plane-bucket.service is not deployed yet.\n'
else
  colmena exec --on "$HOST" -- systemctl start garage-plane-bucket.service
  colmena exec --on "$HOST" -- "systemctl show garage-plane-bucket.service -p Result -p ExecMainStatus | grep -Fxq Result=success && systemctl show garage-plane-bucket.service -p Result -p ExecMainStatus | grep -Fxq ExecMainStatus=0"
  colmena exec --on "$HOST" -- garage bucket info plane-uploads >/dev/null
  colmena exec --on "$HOST" -- "sh -lc 'set -euo pipefail; access_key_id=\$(cat /run/secrets/plane-garage-access-key-id); secret_access_key=\$(cat /run/secrets/plane-garage-secret-access-key); s3cfg=\$(mktemp); src=\$(mktemp); dst=\$(mktemp); headers=\$(mktemp); body=\$(mktemp); trap \"rm -f \\\"\$s3cfg\\\" \\\"\$src\\\" \\\"\$dst\\\" \\\"\$headers\\\" \\\"\$body\\\"\" EXIT; chmod 0600 \"\$s3cfg\"; cat > \"\$s3cfg\" <<EOF
[default]
access_key = \$access_key_id
secret_key = \$secret_access_key
host_base = 127.0.0.1:3900
