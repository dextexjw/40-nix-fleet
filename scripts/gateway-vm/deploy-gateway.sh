...
for required_key in admin-password-hash authentik-bootstrap-password authentik-bootstrap-token authentik-postgresql-password authentik-secret-key beszel-agent-key beszel-agent-token beszel-oidc-client-secret memos-oidc-client-secret checkmate-capture-environment gluetun-control-api-key gluetun-openvpn-password gluetun-openvpn-username restic-password smb-credentials technitium-admin-username technitium-admin-password traefik-cloudflare-dns-api-token; do
