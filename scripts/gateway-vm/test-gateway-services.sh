...
wait_for_remote "Forgejo Authentik discovery endpoint failed" "curl -fsS --resolve auth.jax22.com:443:127.0.0.1 https://auth.jax22.com/application/o/forgejo/.well-known/openid-configuration | grep -Fq '\"issuer\"'"
wait_for_remote "Forgejo Authentik authorize endpoint rejected client or redirect URI" "status=\$(curl -sS -o /tmp/forgejo-authentik-authorize.html -w '%{http_code}' --resolve auth.jax22.com:443:127.0.0.1 'https://auth.jax22.com/application/o/authorize/?client_id=forgejo&redirect_uri=https%3A%2F%2Fforgejo.jax22.com%2Fuser%2Foauth2%2Fauthentik%2Fcallback&response_type=code&scope=openid%20profile%20email&state=fleet-smoke&nonce=fleet-smoke'); case \"\$status\" in 2*|3*) ! grep -Eiq 'invalid(_| |-)?(client|redirect)' /tmp/forgejo-authentik-authorize.html ;; *) echo \"unexpected authorize status \$status\" >&2; exit 1 ;; esac"
wait_for_remote "Paperless Authentik discovery endpoint failed" "curl -fsS --resolve auth.jax22.com:443:127.0.0.1 https://auth.jax22.com/application/o/paperless/.well-known/openid-configuration | grep -Fq '\"issuer\"'"
wait_for_remote "Paperless Authentik authorize endpoint rejected client or redirect URI" "status=\$(curl -sS -o /tmp/paperless-authentik-authorize.html -w '%{http_code}' --resolve auth.jax22.com:443:127.0.0.1 'https://auth.jax22.com/application/o/authorize/?client_id=paperless&redirect_uri=https%3A%2F%2Fpaperless.jax22.com%2Faccounts%2Foidc%2Fauthentik%2Flogin%2Fcallback%2F&response_type=code&scope=openid%20profile%20email&state=fleet-smoke&nonce=fleet-smoke'); case \"\$status\" in 2*|3*) ! grep -Eiq 'invalid(_| |-)?(client|redirect)' /tmp/paperless-authentik-authorize.html ;; *) echo \"unexpected authorize status \$status\" >&2; exit 1 ;; esac"
wait_for_remote "Nextcloud Authentik discovery endpoint failed" "curl -fsS --resolve auth.jax22.com:443:127.0.0.1 https://auth.jax22.com/application/o/nextcloud/.well-known/openid-configuration | grep -Fq '\"issuer\"'"
wait_for_remote "Nextcloud Authentik authorize endpoint rejected client or redirect URI" "status=\$(curl -sS -o /tmp/nextcloud-authentik-authorize.html -w '%{http_code}' --resolve auth.jax22.com:443:127.0.0.1 'https://auth.jax22.com/application/o/authorize/?client_id=nextcloud&redirect_uri=https%3A%2F%2Fnextcloud.jax22.com%2Fapps%2Fuser_oidc%2Fcode&response_type=code&scope=openid%20email%20profile&state=fleet-smoke&nonce=fleet-smoke'); case \"\$status\" in 2*|3*) ! grep -Eiq 'invalid(_| |-)?(client|redirect)' /tmp/nextcloud-authentik-authorize.html ;; *) echo \"unexpected authorize status \$status\" >&2; exit 1 ;; esac"
