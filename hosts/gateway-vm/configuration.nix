  ...
  environment.etc."fleet/gateway-vm.md".text = ''
    ...
    ${lib.concatStringsSep "\n" (map (zoneDomain: "      *.${zoneDomain}") serviceDomains)}
        ...
        Auth model:
          ...
          Native OIDC applications are declared in the exposure catalog. Beszel uses the beszel client with monitoring-users. BookOrbit uses the bookorbit client with media-users and https://bookorbit.jax22.com/oauth2-callback; configure the BookOrbit side in Settings > OIDC / SSO after Authentik provisioning. Memos uses the memos client with productivity-users and https://memos.jax22.com/auth/callback. Gitea uses the gitea client with productivity-users and https://gitea.jax22.com/user/oauth2/authentik/callback. Forgejo uses the forgejo client with productivity-users and https://forgejo.jax22.com/user/oauth2/authentik/callback. Paperless uses the paperless client with productivity-users and https://paperless.jax22.com/accounts/oidc/authentik/login/callback/. Nextcloud uses the nextcloud client with productivity-users and https://nextcloud.jax22.com/apps/user_oidc/code. RustFS Console uses the rustfs-console client with fleet-admins and https://rustfs.jax22.com/rustfs/admin/v3/oidc/callback/authentik.
