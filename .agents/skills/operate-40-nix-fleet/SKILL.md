...
For a stateless service addition or edit, use `scripts/fleet-lifecycle.py plan --action <addition|edit> --host <owner> --service-class stateless` to record scope before editing, then use the same command with `validate` after editing. For an authorized stateful live change, use `run --service-class stateful --mutation-mode live` and keep its phase receipts for safe resumption. The canonical command delegates repository checks and host-specific guarded workflows, emits structured evidence, scopes live mutation to the owner, and never restores automatically.
