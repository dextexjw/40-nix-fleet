...
in
{
  config = mkIf (cfg.enable && affineCfg.enable) {
    assertions = [
      {
        assertion = !cfg.secrets.enable || hasAttr "affine-environment" config.sops.secrets;
        message = "affine-environment must be declared as a SOPS secret when testbed secrets are enabled.";
      }
    ];
    ...
    services.postgresql = {
      enable = true;
      enableTCPIP = true;
      ensureDatabases = [ affineCfg.databaseName ];
      ensureUsers = [
        {
          name = affineCfg.databaseUser;
          ensureDBOwnership = true;
        }
      ];
      authentication = mkAfter ''
        host ${affineCfg.databaseName} ${affineCfg.databaseUser} 127.0.0.1/32 scram-sha-256
      '';
